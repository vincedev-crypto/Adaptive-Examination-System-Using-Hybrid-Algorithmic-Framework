...
public class HomepageController {
    ...
    public String submitExam(@RequestParam Map<String, String> answers, HttpSession session, Model model) {
        Map<Integer, String> key = (Map<Integer, String>) session.getAttribute("correctAnswerKey");
        int score = 0;
        if (key != null) {
            for (var entry : key.entrySet()) {
                String studentAns = answers.get("q" + entry.getKey());
                if (studentAns != null && studentAns.equals(entry.getValue())) score++;
            }
        }
